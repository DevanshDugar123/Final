...
function setup(){
  ...
  play_btn.position( windowWidth/3, windowHeight/3 ) 
  play_btn.size( windowWidth/3 , windowHeight/4 )
  play_btn.hide()

  about_btn = createImg ("assets/About.png")
  about_btn.position( windowWidth/3, windowHeight/1.5 ) 
  about_btn.size( windowWidth/3 , windowHeight/4 )
  about_btn.hide()
...
function gameWin(){
    swal({
        title : "Game Win",
        text: " Congratulations Friend!! You are now the parkour god!! ",
        textAlign : "center",
        imageUrl : "assets/Parkour.gif",
        imageSize : "300x300",
        confirmButtonText : " Restart the game",
...
function fireformSpawn(){
    ...
    fire = createSprite (player.x + 20, -350,width/4,windowHeight/10);
...
function enemySpawn(){
    ...
    enemy = createSprite (windowWidth+350  , random ,width/4,windowHeight/10);
    ...
    enemy.lifetime =400
    //enemy.debug = true;
    enemy.setCollider("rectangle",0,0,enemy.width - 150,enemy.height - 90);
    }
